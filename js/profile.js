...
function startAnimation() {
    ...
    function animate(timestamp) {
        ...
        if (isSquashing) applySquash(timestamp);
...
hamburger.addEventListener("click", () => csukva ? openMenu() : closeMenu());
overlay.addEventListener('click', () => { if (isMobile() && !csukva) hamburger.click(); });
document.addEventListener('keydown', (e) => { if (isMobile() && e.key === 'Escape' && !csukva) hamburger.click(); });
...
window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
        ...
        if (mobile && !wasMobile) { if (!csukva) hamburger.click(); alapAllapotBeallitasa(); }
        else if (!mobile && wasMobile) { if (csukva) hamburger.click(); alapAllapotBeallitasa(); }
    }, 100);
});
...
export function validateImage(file) {
    ...
    if (!allowedTypes.includes(file.type)) throw new Error('Csak JPG, PNG, GIF és WebP formátumok engedélyezettek');
    if (file.size > maxSize) throw new Error('A kép mérete nem haladhatja meg az 5MB-ot');
    return true;
}
...
export function showPasswordMessage(text, type = 'success') {
    ...
    if (type === 'success') setTimeout(() => container.querySelector('.message')?.remove(), 3000);
}
...
const EYE_OPEN_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" aria-label="Mutasd a jelszót"><path fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" d="M1.5 12s4.5-7.5 10.5-7.5S22.5 12 22.5 12s-4.5 7.5-10.5 7.5S1.5 12 1.5 12z"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.6"/></svg>`;
const EYE_CLOSED_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" aria-label="Rejtsd a jelszót"><path fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" d="M1.5 12s4.5-7.5 10.5-7.5 10.5 7.5 10.5 7.5-1.8 3-5.7 4.8M3 3l18 18"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.6"/></svg>`;
...
export function adjustDropdownToForm(dropdown) {
    ...
    if (spaceBelow > 100) { maxHeight = Math.min(spaceBelow - 10, 400); position = 'below'; }
    else if (spaceAbove > 100) { maxHeight = Math.min(spaceAbove - 10, 400); position = 'above'; }
    else { maxHeight = Math.min(Math.max(spaceBelow, spaceAbove) - 10, 400); position = spaceBelow >= spaceAbove ? 'below' : 'above'; }
    ...
    if (searchContainer && dropdownList) {
        ...
        const available = maxHeight - parseInt(styles.paddingTop || 0) - parseInt(styles.paddingBottom || 0) - searchContainer.offsetHeight;
...
function initializeSingleDropdown(dropdown, sportokHTML, preSelectedSport = null) {
    ...
    if (!clearBtn) {
        ...
        clearBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="4" x2="20" y2="20"/><line x1="20" y1="4" x2="4" y2="20"/></svg>`;
    ...
    clearBtn.onclick = (e) => {
        ...
        setTimeout(() => { handleDynamicTags(sportokHTML); initializeAccordions()?.updateHeight(); }, 50);
        return false;
    };
...
function handleDynamicTags(sportokHTML) {
    ...
    if (dropdowns.length >= 2) {
        for (let i = dropdowns.length - 1; i >= 1; i--) {
            ...
            const currEmpty = (curr.querySelector('.search-dropdown-btn')?.textContent || '').includes("Válassz sportot");
            const prevEmpty = (prev.querySelector('.search-dropdown-btn')?.textContent || '').includes("Válassz sportot");
            if (currEmpty && prevEmpty && curr.id !== 'dropdown3') { curr.remove(); break; }
        }
    }
    const updated = container.querySelectorAll('.search-dropdown');
    if (updated.length > 0) {
        const lastText = updated[updated.length - 1].querySelector('.search-dropdown-btn')?.textContent || '';
        if (!lastText.includes("Válassz sportot") && lastText.trim()) addNewDropdown(sportokHTML);
    }
}
...
async function getTeamStats(teamId, days = 30) {
    try {
        ...
        return {
            dailyData: Object.keys(dailyStats).map(date => ({
                date,
...
function renderTeamChart(teamId, dailyData) {
    ...
    teamCharts[teamId] = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            scales: {
                x: { display: true, grid: { display: false }, ticks: { maxRotation: 0, maxTicksLimit: 10, font: { size: 10 }, color: '#666' } },
                y: { display: true, beginAtZero: true, ticks: { precision: 0, font: { size: 10 }, color: '#666' }, grid: { color: 'rgba(0, 0, 0, 0.05)' } }
            }
        }
    });
}
...
function renderDashboard(teamsData) {
    ...
    if (teamsData.length === 0) {
        container.innerHTML = `
            <div style="grid-column:1/-1;text-align:center;padding:3rem 1rem;color:#7d6558;">
                <p style="margin:0;font-size:0.9rem">Még nincsenek csapataid.</p>
            </div>`;
        return;
    }
        ...
        return `
            <div class="team-chart-container" data-team-id="${team.$id}">
                <div class="team-chart-header">
                    ...
                    <span class="team-chart-badge">${total} megtekintés</span>
                </div>
...
export async function initDashboard(currentUser, userTeams) {
    if (typeof databases === 'undefined') { console.error('Appwrite databases nincs definiálva!'); return; }
    if (typeof Chart === 'undefined') { console.error('Chart.js nincs betöltve!'); return; }
    try {
...
const ICON = {
    ...
    team:   `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3 20c0-3.3 2.7-6 6-6s6 2.7 6 6"/><circle cx="18" cy="9" r="2.5"/><path d="M13.5 19.5c0-2.5 2-4.5 4.5-4.5s4.5 2 4.5 4.5"/></svg>`,
    image:  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`,
};

function szekció(ikon, cím, tartalom, extraClass = '') {
    return `
        <div class="szekció ${extraClass}">
            <div class="szekció-fejléc">${ikon}<h3>${cím}</h3></div>
            ${tartalom}
        </div>`;
}
...
function formGroup(label, inputHTML) {
    return `
        <div class="form-group-kompakt">
            <label class="label-kompakt">${label}</label>
            ${inputHTML}
        </div>`;
}
...
function showMyTeamsView() {
    ...
    document.getElementById('jobb-oldal').innerHTML = `
        <div class="üdvözlő-sáv">
            <div>
                <h2>Szia, ${currentUser.name}!</h2>
                <p>Itt látod csapataid statisztikáit az elmúlt 30 napból.</p>
            </div>
        </div>
        <div id="dashboard-container">
            <div style="grid-column:1/-1;color:#7d6558;font-size:0.85rem;padding:1rem 0;">
                <svg style="width:20px;height:20px;vertical-align:middle;margin-right:6px;animation:spin 1s linear infinite" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" stroke-dasharray="30 60"/></svg>
                Betöltés...
            </div>
        </div>
...
function showProfileView() {
    loadUserTeams();
    document.getElementById('jobb-oldal').innerHTML = `
        <div class="nézet-fejléc">
            <h2>Beállítások</h2>
            <p>Fiókod adatainak kezelése</p>
        </div>

        <div class="profil-meta">
            <div class="profil-avatar">${initials(currentUser.name)}</div>
        ...
        <div id="profil-container">
            ${szekció(ICON.user, 'Személyes adatok', `
                <form class="szekció-grid" id="name-form">
                    ${formGroup('Teljes név *', `<input class="input-kompakt" type="text" id="profile-name" value="${currentUser.name}" required>`)}
                    ${formGroup('Email cím', `<input class="input-kompakt" type="email" value="${currentUser.email}" disabled>`)}
                    <div style="display:flex;justify-content:flex-end;margin-top:0.25rem">
                        <button type="submit" id="name-submit-btn" class="btn-mentés">Mentés</button>
                    </div>
                    <div id="name-message-container"></div>
                </form>
            `)}

            ${szekció(ICON.lock, 'Jelszó megváltoztatása', `
                <form class="szekció-grid" id="password-form">
                    ${buildPasswordField('current-password', 'Jelenlegi jelszó *', 'Jelenlegi jelszó')}
                    ${buildPasswordField('new-password', 'Új jelszó *', 'Új jelszó')}
                    ${buildPasswordField('confirm-password', 'Jelszó megerősítése *', 'Jelszó megerősítése')}
                    <div style="display:flex;justify-content:flex-end;margin-top:0.25rem">
                        <button type="submit" id="password-submit-btn" class="btn-mentés" enabled>Jelszó módosítása</button>
                    </div>
                    <div id="password-message-container"></div>
                </form>
            `)}

            <div class="szekció danger-zone fullwidth">
                <div class="szekció-fejléc">${ICON.danger}<h3>Veszélyzóna</h3></div>
                <p>A fiók törlése <strong>visszavonhatatlan</strong>. Minden csapatod, adatod és beállításod véglegesen törlődik.</p>
                <button type="button" id="delete-account-btn" class="btn-törlés">Fiók törlése</button>
            </div>
        </div>
    `;

    setupPasswordChangeHandlers();
    setupNameChangeHandler(initials);
...
function initials(words) {
    return words
        ? words.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
        : '?';
}
...
async function showTeamEditView(teamId) {
    ...
    document.getElementById('jobb-oldal').innerHTML = `
        <div class="csapat-header">
            <div class="csapat-header-info">
                <p>Csapat szerkesztése</p>
                <h2>${team.nev}</h2>
            </div>
        </div>
        ...
        <div id="csapat-logo-container">
            <img class="csapat-logo" src="${team.kep}" alt="${team.nev} logója"
                onerror="this.style.display='none'" style="${team.kep ? '' : 'display:none'}">
            ${!team.kep ? '<p class="no-image">Nincs logó</p>' : ''}
    ...
    initializeSzerkesztoHandlers(team,teamId);
    disableEnterSubmission();
...
async function showNewTeamView() {
    ...
    try {
        ...
        while (true) {
            const res = await databases.listDocuments(DB_ID, 'sportok', [Query.limit(100), Query.offset(offset)]);
        ...
        console.error('Hiba a sportok betöltésekor:', error);
        jobbOldal.innerHTML = `<div class="message error">Hiba történt az adatok betöltése során: ${error.message}</div>`;
        return;
    }
    ...
    jobbOldal.innerHTML = `
        <div class="nézet-fejléc">
            <h2>Új csapat</h2>
            <p>Töltsd ki az adatokat az új csapat létrehozásához</p>
        </div>

        <div id="message-container"></div>

        <div class="szekció-grid szekció-grid-2" style="gap:1rem;margin-bottom:1rem">
            ${szekció(ICON.edit, 'Alapadatok', `
                <div class="szekció-grid">
                    ${formGroup('Csapat neve *', `<input class="input-kompakt" type="text" name="nev" placeholder="Pl. Budapest FC" required>`)}
                    ${formGroup('Email *', `<input class="input-kompakt" type="email" name="email" placeholder="info@csapat.hu" required>`)}
                    ${formGroup('Telefon *', `<input class="input-kompakt" type="text" name="telefon" id="telefon" pattern="^\\S+$" maxlength="12" placeholder="+36301234567" required>`)}
                    ${formGroup('Weboldal', `<input class="input-kompakt" type="text" name="weboldal" placeholder="https://csapat.hu">`)}
                    ${formGroup('Tagdíj', `<div style="position:relative"><input class="input-kompakt" type="text" name="tagdij" placeholder="5000" style="padding-right:2.5rem"><span style="position:absolute;right:0.75rem;top:50%;transform:translateY(-50%);font-size:0.8rem;color:#7d6558">Ft</span></div>`)}
                </div>
            `)}
            ${szekció(ICON.map, 'Székhely', `
                <div class="szekció-grid">
                    ${formGroup('Irányítószám *', `<input class="input-kompakt" type="text" name="iranyitoszam" minlength="4" maxlength="4" placeholder="1234" required>`)}
                    ${formGroup('Város *', `<input class="input-kompakt" type="text" name="varos" placeholder="Budapest" required>`)}
                    ${formGroup('Utca *', `<input class="input-kompakt" type="text" name="utca" placeholder="Fő utca" required>`)}
                    ${formGroup('Házszám *', `<input class="input-kompakt" type="text" name="hazszam" placeholder="12" required>`)}
                </div>
            `)}
        </div>

        <form class="form" id="form" enctype="multipart/form-data" style="display:contents">
            <div class="szekció" style="margin-bottom:1rem">
                <div class="szekció-fejléc">${ICON.team}<h3>Sportok</h3></div>
                <div id="cimkek-container">
            ...
            ${szekció(ICON.image, 'Logó feltöltése', `
                <div id="kep-feltoltes">
                    <input type="file" name="kep" accept="image/*" id="kepInput">
                    <img id="kepPreview" src="" alt="" style="display:none;margin-top:0.75rem;max-width:120px;border-radius:8px">
                </div>
            `)}

            <div style="display:flex;align-items:center;justify-content:space-between;margin-top:0.5rem">
                <small style="color:#7d6558">* A mező kitöltése kötelező</small>
                <button type="button" id="submit-btn" class="btn-mentés" style="padding:0.65rem 1.75rem;font-size:0.95rem">Csapat létrehozása</button>
            </div>
        </form>
    `;
...
function buildPasswordField(id, label, placeholder) {
    const eyeSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" aria-label="Mutasd a jelszót"><path fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" d="M1.5 12s4.5-7.5 10.5-7.5S22.5 12 22.5 12s-4.5 7.5-10.5 7.5S1.5 12 1.5 12z"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.6"/></svg>`;
    return `
        <div class="form-group-kompakt">
                <label  class="label-kompakt">${label}</label>            
                <input class="input-kompakt" type="password" id="${id}" placeholder="${placeholder}" required>
...
async function changePassword(e) {
    ...
    if(newPassword === confirmPassword) {
        ...
        const msg = "Az új jelszó és a megerősítés nem egyezik";
        showPasswordMessage(`❌ ${msg}`, 'error');
        submitBtn.disabled = false;
    }
}
...
function setupNameChangeHandler() {
    document.getElementById('name-form')?.addEventListener('submit', async (e) => {
        ...
        const container = document.getElementById('name-message-container');
        if (!name) return;
        ...
        try {
            ...
            document.querySelector("#jobb-oldal > div.profil-meta > div.profil-meta-info > h3").innerHTML = `${name}`;
            document.querySelector("#jobb-oldal > div.profil-meta > div.profil-avatar").innerHTML = `${initials(currentUser.name)}`;
            showMessage(container, '✅ Név sikeresen frissítve!', 'success');
        } catch (error) {
            const msg = await translateErrorMessage(error);
...
function initializeImageHandlers(teamId) {
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        if (!file) return;
        try { validateImage(file); await uploadNewTeamImage(file, teamId); }
...
async function initializeTeamForm() {
    ...
    if (!form) { setTimeout(initializeTeamForm, 100); return; }
    ...
    if (kepInput && kepPreview) kepInput.addEventListener("change", e => handleImagePreview(e, kepPreview));
...
function handleImagePreview(event, kepPreview) {
    ...
    reader.onload = function (e) {
        ...
        deleteIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="4" x2="20" y2="20"/><line x1="20" y1="4" x2="4" y2="20"/></svg>`;
        deleteIcon.style.cssText = `position:absolute;top:24px;background:#dc3545;border-radius:50%;padding:4px;cursor:pointer;z-index:100;width:24px;height:24px;display:flex;align-items:center;justify-content:center;`;
        deleteIcon.style.left = kepPreview.clientWidth + 'px';
        deleteIcon.onclick = () => { kepPreview.src = ''; kepPreview.style.display = 'none'; event.target.value = ''; deleteIcon.remove(); };
...
async function handleFormSubmit(event, form, messageContainer) {
    ...
    if (fields.some(f => !document.querySelector(`input[name="${f}"]`)?.value.trim())) {
        showMessage(messageContainer, 'Minden kötelező mezőt tölts ki!', 'error');
        return;
    }

    const selectedSports = Array.from(document.querySelectorAll('.search-dropdown-btn'))
        .map(btn => btn.textContent.trim()).filter(t => t && !t.includes('Válassz sportot'));
    const uniqueSports = [...new Set(selectedSports)];

    if (uniqueSports.length === 0) { showMessage(messageContainer, 'Legalább egy sportot válassz ki!', 'error'); return; }
    ...
    let newTeam = null; // Előre definiáljuk
    ...
    try {
        ...
        const formData = {
            nev: document.querySelector('input[name="nev"]').value,
            email: document.querySelector('input[name="email"]').value,
            telefon: document.querySelector('input[name="telefon"]').value,
            weboldal: document.querySelector('input[name="weboldal"]').value || '',
            iranyitoszam: document.querySelector('input[name="iranyitoszam"]').value,
            varos: document.querySelector('input[name="varos"]').value,
            utca: document.querySelector('input[name="utca"]').value,
            hazszam: document.querySelector('input[name="hazszam"]').value,
            cimkek: uniqueSports,
        ...
        // ELMENTJÜK A LÉTREHOZOTT DOKUMENTUMOT
        newTeam = await databases.createDocument(DB_ID, 'csapatok', 'unique()', formData);
        ...
        document.getElementById('cimkek-container').innerHTML = `
            <div class="search-dropdown" id="dropdown3">
                <button class="search-dropdown-btn" type="button">Válassz sportot</button>
                <div class="search-dropdown-content">
                    <div class="search-container"><input type="text" class="search-input" placeholder="Keresés..."></div>
                    <div class="dropdown-list"></div>
                </div>
            </div>`;
        initializeAllDropdowns('');
        
        // SIKER ESETÉN - átirányítás a szerkesztő nézetbe
        if (newTeam?.$id) {
            userTeams.push(newTeam);      // ← hozzáadás a tömbhöz
            renderTeamButtons();
            showTeamEditView(newTeam.$id);
        }
        ...
        // A gomb visszaállítása mindig megtörténik
        if (submitBtn) {
